var g_alfabeto = [];  //Declaração das Variaveis
var g_estados = [[]];
...
$('#token').keypress(function (e) 
{
    ...
    {
        let elemento = $(this);
        let tokens = elemento.val().split(' '); //Gravar a palavra digitada na variavel token
        elemento.val('');
        
        criar_alfabeto(tokens); // Chamar o método criar alfabeto 
        g_estados = [[]];
...
function criar_alfabeto(tokens) 
{
    ...
    {
        g_alfabeto.push(token); //Adiciona ao array o token digitado
        $('#alfabeto').append(
...
{
    g_alfabeto.splice( g_alfabeto.indexOf(token), 1 ); // Removendo da lista o token ( a palavra )
    g_estados = [[]];
    $('#span-'+token).remove();
    identificar_estados_do_automato(); // Desenha a tabela novamente 
    reiniciar_variaveis();
    desenhar_tabela();
    $('#palavra').val('');
}

function identificar_estados_do_automato() // Cada vez que pega os keypress do input, 
//vai criando os estado gerais e o estado atual.
//nesse estado atual e na lista dos estados gerais, vai ser mapeado a tabela. 
//Cada vez que for digitado uma letra o automado verifica se é estado existente, 
//e ela é responsável por chamar todas as funções. 
{    
    var estado = 0;
